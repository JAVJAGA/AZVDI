...
resource "azurerm_network_interface" "rdsh" {
  count                     = "${var.rdsh_count}"
  name                      = "${var.prefix}${format("%002d", count.index + 1)}-nic"
  location                  = var.azure_location
  resource_group_name       = var.resourcegroupname
  

  ip_configuration {
    name                          = "nic${format("%002d", count.index + 1)}_config"
...
resource "azurerm_virtual_machine" "windows_vm" {
  count                 = "${var.rdsh_count}"
  name                  = "${var.prefix}${format("%002d", count.index + 1)}"
  location              = var.azure_location
  resource_group_name   = var.resourcegroupname
  ...
  storage_os_disk {
    name              = "${lower(var.prefix)}${format("%002d", count.index + 1)}"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "${var.prefix}${format("%002d", count.index + 1)}"
