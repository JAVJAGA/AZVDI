        ...
        stage('Init') {
            ...
            steps {
                ...
                withCredentials([azureServicePrincipal(TEMP_CREDENTIALS)]) {
                    ...
                    terraform version
                    terraform init -upgrade -migrate-state -no-color -backend-config="backend-${params.environment}.tfvars" -var client_secret=${ARM_CLIENT_SECRET} \
         ...
         stage('Plan') {
            ...
            steps {
                ...
                withCredentials([azureServicePrincipal(TEMP_CREDENTIALS)]) {
                    ...
                    sh """
                    terraform plan -replace -no-color -out tfplan -var-file="terraform-${params.environment}.tfvars" -var client_secret=${ARM_CLIENT_SECRET} \
                            -var subscription_id=${ARM_SUBSCRIPTION_ID} \
                            -var tenant_id=${ARM_TENANT_ID} \
                            -var client_id=${ARM_CLIENT_ID}
                    """
